...
def crossover(parent_alpha, parent_beta):
    ...
    def generate_child(primary_parent, secondary_parent):
        ...
        # Fill remaining slots from the secondary parent
...
# A local 2-opt search heuristic applied within a single route
    ...
    while was_improved:
        ...
        for i in range(1, len(current_best_route) - 2):
            for j in range(i + 1, len(current_best_route) - 1):
                # Calculate if reversing the segment reduces the total distance
...
# Moves a customer from one van's route to another using Vehicle for capacity validation.
    ...
    while was_improved:
        ...
        for route_1_index, route_1 in enumerate(routes_list):
            for route_2_index, route_2 in enumerate(routes_list):
                ...
                # Build a Vehicle representing the destination route for explicit capacity checks
